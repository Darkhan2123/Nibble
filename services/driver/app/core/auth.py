...
async def is_driver_owner(driver_id: str, user_id: str) -> bool:
    """Check if a user is the owner of a driver profile."""
    ...
    return driver.get("user_id") == user_id

def driver_owner_or_admin():
    """Check if user is the driver owner or admin."""
    async def check_ownership(
        ...
        # If driver_id is not provided, check if user has driver role
        ...
        # If driver_id is provided, check if user owns this driver profile
        is_owner = await is_driver_owner(driver_id, user_info.get("user_id"))
        if is_owner:
            return user_info
                
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You don't have permission to access this driver profile",
        )
